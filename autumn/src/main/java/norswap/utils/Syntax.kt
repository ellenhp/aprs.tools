 ...
 */
inline operator fun <T> Boolean.rangeTo(f: () -> T): T? = then(f)
 ...
 */
inline infix fun <T> Boolean.then(f: () -> T): T? = if (this) f() else null
 ...
 * inference is dumb.
 */
inline fun <reified T, R> Any?.when_is(f: (T) -> R): R? = if (this is T) f(this) else null
 ...
 */
inline fun <T : Any, R : Any> T?.inn(f: (T) -> R): R? = if (this != null) f(this) else null
 ...
 */
inline fun <T : Any> attempt(f: () -> T): T? = try {
    f()
} catch (_: Exception) {
    null
}
