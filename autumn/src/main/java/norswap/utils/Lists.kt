 ...
 */
inline fun <reified T> Iterable<*>.first_instance(): T = filter { it is T } as T
 ...
 */
fun List<*>.lines(): String = joinToString(separator = ",\n ", prefix = "[", postfix = "]")
 ...
 */
fun <T> List<T>.rest(n: Int = 1): List<T> = subList(n, size)
 ...
 */
fun <T> List<T>.except(n: Int = 1): List<T> = subList(0, size - n)
 ...
 */
fun <T : Any> maybe_list(item: T?): List<T> = if (item == null) emptyList() else listOf(item)
 ...
 */
fun <T> maybe_list(list: List<T>?): List<T> = list ?: emptyList()
