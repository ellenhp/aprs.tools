 ...
 */
fun Grammar.char_any(): Boolean = char_pred { it != '\u0000' }
 ...
 */
fun Grammar.char_range(start: Char, end: Char): Boolean = char_pred { it in start..end }
 ...
 */
fun Grammar.char_set(vararg chars: Char): Boolean = char_pred { chars.contains(it) }
 ...
 */
fun Grammar.char_set(chars: String): Boolean = char_pred { chars.contains(it) }
 ...
 */
fun Grammar.alpha(): Boolean = char_pred { it in 'a'..'z' || it in 'A'..'Z' }
 ...
 */
fun Grammar.alphanum(): Boolean = char_pred { it in 'a'..'z' || it in 'A'..'Z' || it in '0'..'9' }
 ...
 */
fun Grammar.digit(): Boolean = char_pred { it in '0'..'9' }
 ...
 */
fun Grammar.hex_digit(): Boolean = char_pred { it in 'a'..'f' || it in 'A'..'F' || it in '0'..'9' }
 ...
 */
fun Grammar.octal_digit(): Boolean = char_pred { it in '0'..'7' }
 ...
 */
fun Grammar.space_char(): Boolean = char_pred(Char::isWhitespace)
 ...
 */
fun Grammar.java_iden(): Boolean = transact_contain(ExpectedIdentifier) b@{
    if (!text[pos].isJavaIdentifierStart())
        return@b false
    var c: Char
    do {
        c = text[++pos]
    } while (c.isJavaIdentifierPart() && c != '\u0000')
    return@b true
}
 ...
 * ASCII characters.
 */
fun Grammar.ascii_java_iden(): Boolean = transact_contain(ExpectedIdentifier) b@{
